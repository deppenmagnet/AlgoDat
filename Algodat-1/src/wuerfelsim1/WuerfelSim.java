...
public class WuerfelSim {
	...
	static void teste_wuerfel(double[] wahrscheinlichkeitsverteilung){
		...
		int wurf_zaehler = 0;
		// Werfen und Testen!
		// Da ich nur drei Nachkommastellen brauche kann ich prima mit Integern
		// arbeiten. So vermeide ich beim Vergleichen Schwierigkeiten mit
		// den Gleitkomma-Ungenauigkeiten und kann die schnelle Integer-
		...
		while(!abbruch){
			...
			// Abbruchbedingung
			// Alle 12 Werte von dist und dist_real müssen gleich 
			// sein.
			int anzahl_korrekter_ergebnisse = 1; // da 1 nie vorkommen kann, 
												 // kann ich bei zwei beginnen
		...
		for (int i = 2; i < dist.length; i++)
			System.out.println(i+": "+wuerfe[i]+"mal -> "+dist_real[i]+ " -> " 
								+ dist [i]);
	}
